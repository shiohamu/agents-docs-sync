# ステップ12: AgentsGeneratorの変更

## 目的

新しい仕様に合わせて`AgentsGenerator`を大幅に変更します。
...
1. `collect_dependencies`メソッドで`json.load(f)`の後に`()`が抜けている
2. `collect_ci_cd_info`メソッドで`workflows_dir`のパスが間違っている（`'workflows'`の前に`'/'`が必要）
3. `_generate_setup_section`メソッドで文字列の引用符が閉じられていない箇所がある

これらは実装時に修正してください。

## 確認事項

- [ ] `AgentsGenerator`が正しく変更されているか
- [ ] `ProjectInfoCollector`が正しくインポートされているか
- [ ] すべてのセクションが正しく生成されるか
- [ ] エラーハンドリングが適切か
- [ ] 既存のインターフェース（`generate()`メソッド）が維持されているか
- [ ] API/ローカルLLMの両パターンが正しく表示されるか

## 次のステップ

このステップが完了したら、[ステップ13: AGENTS.md生成のテストと検証](./step13-agents-testing.md)に進んでください。
