...
agents-docs-sync は、コードベースに追加・変更があるたびに自動的にテストを実行し、ドキュメント（docstring からの API リファレンスや README の補完）を生成して **AGENTS.md** を最新化する CI パイプラインです。  
主な機能とワークフローは次の通りです。

- **テスト実行** – `pytest` とその拡張（coverage, mock 等）でユニット／統合テストを走らせ、失敗した場合にビルド全体が停止します。  
- **コード解析 & ドキュメント生成** – Python の `pyyaml` でエージェント定義ファイル (`agents.yaml`) を読み込み、各クラス・関数の docstring と型ヒントをもとに Markdown / ReST ファイルへ変換します。  
- **AGENTS.md 自動更新** – 上記生成物から最新情報（API 名、パラメータ説明、使用例）を抽出し、既存の AGENTS.md を差分比較して上書き保存します。この際 `sed` / `awk` などシェルスクリプトで構造化されたブロックのみを書き換えます。  
- **パッケージ管理** – 全て Python は `uv`（高速依存解決）を利用し、必要ライブラリは `pyyaml>=6.0`, `pytest>=7.4`, `pytest-cov>=4.1`, `pytest-mock>=3.11` 等で固定。  
- **CI 環境** – GitHub Actions / GitLab CI など任意の CI サーバーに簡単に組み込めるよう、`.github/workflows/pipeline.yml` にサンプル設定を付属しています。

このプロジェクトは「変更＝自動更新」モデルでドキュメント整合性とテスト品質を同時に保ちます。エージェント開発者がコードを書くだけで、最新の API 仕様書が即座に反映されるため、レビューや導入作業がスムーズになります。
**使用技術**: python, shell
## プロジェクト構造
```
├── docgen//
...
*このAGENTS.mdは自動生成されています。最終更新: 2025-12-12 19:51:15*