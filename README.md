...
プロジェクトルートに `docgen/config.yaml` ファイルを配置することで、ドキュメント生成の動作をカスタマイズできます。

#### デバッグ設定

デバッグ情報を有効/無効にすることができます：

```yaml
# デバッグ設定（オプション）
debug:
  # デバッグモードを有効にするかどうか（デフォルト: false）
  # trueに設定すると、詳細なデバッグ情報が出力されます
  enabled: false
```

デバッグモードを有効にすると、設定読み込みやLLM通信などの詳細なログが出力されます。通常の使用時は無効にしておくことを推奨します。

#### その他の設定

言語検出、出力パス、LLM設定などの詳細な設定については `docgen/config.yaml.sample` を参照してください。
...
*このREADME.mdは自動生成されています。最終更新: 2025-11-24 13:15:17*
