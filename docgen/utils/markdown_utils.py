...
def format_commands_with_package_manager(
    commands: list[str], package_managers: dict[str, str], language: str, max_commands: int = 5
) -> list[str]:
    """
    ...
    for cmd in commands[:max_commands]:
        display_cmd = cmd
        # Adjust for uv package manager
        if language in ["python"] and package_managers.get("python") == "uv":
            if cmd.startswith("python") and not cmd.startswith("uv run"):
                display_cmd = f"uv run {cmd}"
            elif cmd.startswith("pytest") and not cmd.startswith("uv run"):
                display_cmd = f"uv run {cmd}"
        lines.append(display_cmd)

    if len(commands) > max_commands:
        lines.append("# ... その他のコマンド")
...
class MarkdownSectionBuilder:
    ...
    @staticmethod
    def build_code_block(commands: list[str], language: str = "bash") -> list[str]:
        """
