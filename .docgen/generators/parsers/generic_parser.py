...
class GenericParser(BaseParser):
    ...
    COMMENT_PATTERNS = {
        ...
        'java': (r'/\*\*\s*\n(.*?)\*/', r'public\s+(?:static\s+)?(?:.*?\s+)?(\w+)\s*\(', r'public\s+class\s+(\w+)'),
        ...
        try:
            ...
            for match in re.finditer(comment_pattern, content, re.DOTALL):
                ...
                if func_match:
                    ...
                    signature = self._extract_signature(content, start_pos + func_match.start(), name, 'function')
